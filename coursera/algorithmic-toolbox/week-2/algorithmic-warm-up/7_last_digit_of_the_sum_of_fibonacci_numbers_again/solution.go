...
func fibonacciPartialSum(m, n int64) int64 {
	subset, full := fibonacciModulo(m+1, 10), fibonacciModulo(n+2, 10)
	if full > subset {
		return full - subset
	}
	return (10 - subset + full) % 10
}
...
func main() {
	...
	safe(fmt.Fprintln(os.Stdout, fibonacciPartialSum(a, b)))
}
